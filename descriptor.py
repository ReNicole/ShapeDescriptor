...
def gridSampleXU(mesh):
	...
	grid_xu = griddata((spsam[:,1],spsam[:,2]), spsam[:,0], (xi[None,:],yi[:,None]), method='nearest')
	return grid_xu

def gridSampleYU(mesh):
	""" 
	...
	.. attention: the compution requires the norm vector of each sample point
	    however, the sample method provided by trimesh only returns the coordinate of the points
		which is not convenient for this application
		so the sample part is rewritten 
		for more information of the sample algorithm, please refer to
		1.trimesh.sample.sample_surface(the source code)
		https://github.com/mikedh/trimesh/blob/master/trimesh/sample.py#L32
		2.Triangle Point Picking(the algorithm)
		http://mathworld.wolfram.com/TrianglePointPicking.html
	"""
	# get enough sample points to cover the sphere S^2 (-180,180)*(-90,90) (degree based)
	sample_num = 360 * 180 * 3
	# sample process: get the face index of the sample points 
	area = mesh.area_faces
	area_sum = np.sum(area)
	area_cum = np.cumsum(area)
	face_pick = np.random.random(sample_num) * area_sum
	face_index = np.searchsorted(area_cum, face_pick)
	# sample process: get (v2-v0),(v1-v0) of each triangle (pull triangles into the form of an origin + 2 vectors)
	tri_origins = mesh.triangles[:,0]
	tri_vectors = mesh.triangles[:,1:].copy()
	tri_vectors -= np.tile(tri_origins,(1,2)).reshape((-1,2,3))
	# sample process: pull the vectors for the faces we are going to sample from
	tri_origins = tri_origins[face_index]
	tri_vectors = tri_vectors[face_index]
	# sample process: randomly generate two 0-1 scalar components to multiply edge vectors by
	random_lengths = np.random.random((len(tri_vectors),2,1))
	# sample process: points will be distributed on a quadrilateral if we use 2 0-1 samples
	...
	# align
	samples = samples - np.array([mesh.center_mass] * len(samples))
	# get the cross product of (v2-v0),(v1-v0) of each face corresponding to the sample points
	sample_cross = mesh.triangles_cross[face_index]
	...
	xi = np.linspace(-np.pi,np.pi,360)
	yi = np.linspace(-0.5*np.pi,0.5*np.pi,180)
	grid_yu = griddata((spsam[:,1],spsam[:,2]), yu, (xi[None,:],yi[:,None]), method='nearest')
	return grid_yu

def gridSampleRU(mesh):
	""" 
	...
	xi = np.linspace(-np.pi,np.pi,360)
	yi = np.linspace(-0.5*np.pi,0.5*np.pi,180)
	grid_ru = griddata((spsam[:,1],spsam[:,2]), ru, (xi[None,:],yi[:,None]), method='nearest')
	return grid_ru	

def showSampleXU(mesh):
	"""
	visualize the samples of x(u) generated by the func::gridSampleXU
	"""
	fig,ax = plt.subplots(1,1,figsize=(10,5))
	ax.imshow(gridSampleXU(mesh),extent=(-180,180,-90,90),cmap='viridis')
	plt.show()

def showSampleYU(mesh):
	"""
	visulize the samples of y(u) generated by the func::gridSampleYU
	"""
	fig,ax = plt.subplots(1,1,figsize=(10,5))
	ax.imshow(gridSampleYU(mesh),extent=(-180,180,-90,90),cmap='viridis')
	plt.show()	

def descriptorRS(mesh, coef_num_sqrt=13):
	""" 
	...
	---------------------------------------------------------------------------------
	para::mesh: the mesh required to generate a shape descriptor
...
def descriptorSS(mesh, coef_num_sqrt=13):
	"""
...
def descriptorCS(mesh, coef_num_sqrt=13):
	"""
...
def showReXU2d(mesh, coef_num_sqrt=13):
	"""
	visualize the reconstruction of x(u) with truncated spherical system 
	para::mesh: the given mesh (trimesh format)
	para::coef_num_sqrt: the square root of desired number of the dimensions of the 
	    shape of the mesh which is also the number of the truncated coefficients
	"""
	# get the sample value of x(u)
...
def showReYU2d(mesh, coef_num_sqrt=13):
	"""
...
def showScatterReXU3d(mesh,coef_num_sqrt=13):
	"""
